...
def mainGame(movementInfo):
    ...
    playerMaxVelY =  18   # max vel along Y, max descend speed
...
def getRandomPipe():
    ...
    if len(array) < 59:
        while usedBefore == True:
            PIPEGAPSIZE  = random.randint(90,150)
            if PIPEGAPSIZE in array:
                usedBefore = True
                #print("used before")
            elif not PIPEGAPSIZE in array:
                usedBefore = False
                array.append(PIPEGAPSIZE)
                #print("not used")
    else:
        PIPEGAPSIZE  = random.randint(100,170)
    #print(PIPEGAPSIZE)
